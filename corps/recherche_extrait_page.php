  ...
  <form id="myForm" action="" method="get" style=" text-align: center;">
    ...
    <tbody>
      ...
      foreach ($dbh->query("SELECT * FROM echantillon ORDER BY ech_code_echantillon") as $row) {
        echo '
        <tr>
        <td><input class="echantillon_nouveau specimen_nouveau expedition_existant" type="radio" name="echantillon" value="'.urldecode($row[0]).'"';if (isset($_GET['echantillon']) && $row[0] == $_GET['echantillon']) echo "checked"; ;echo '></td>
